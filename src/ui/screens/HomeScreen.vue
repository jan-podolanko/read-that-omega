...
<template>
    <main id="homeScreen">
        <div id="posts">
            <select>
                <option v-for="subject in state.subjects" style="color:black" :v-model="filter">{{
                    subject.subject
                    }}
                </option>
            </select>
            <button @click="filterBySubject(filter)">filter by subject</button>
            <section class="post" v-for="post in state.posts">
                ...
                <div v-if="post.imageURL !== null" class="post-image">
                    <img alt = "image post" :src="`${post.imageURL}`"/>
                </div>
